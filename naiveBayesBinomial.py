...
def testBayes(X,y):
	...
	sizeOfAcc = 500 # number of times to capture accuracy
	...
	for times in range(0,sizeOfAcc):
		...
		X_train_Int = X_train[:,0:shouldBeEven]
		X_train_OneZero = X_train[:,shouldBeEven:]
		X_test_OneZero = X_test[:,shouldBeEven:]
		n,d = X_train_Int.shape

		# smooth X_train_Int
		X_train_Int = X_train_Int + 1

		featureGivenClass = np.zeros((Cn,d), dtype=float)
		classProb = np.zeros(Cn, dtype=float)

		for i in range(0,Cn):
			Cin = X_train_Int[y_train==i].sum()
			classProb[i] = len(X_train_Int) # number of feature instances
			featureGivenClass[i]=(X_train_Int[y_train==i].sum(axis=0))*(1/Cin)
